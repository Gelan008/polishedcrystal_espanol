TrainerClassNames::
	list_start
	li "Entre. <PK><MN>" ; CAL
	li "Entre. <PK><MN>" ; CARRIE
	li "Entre. <PK><MN>" ; JACKY
	li "Entre. <PK><MN>" ; EUNA
	li "Líder"           ; FALKNER
	li "Líder"           ; BUGSY
	li "Líder"           ; WHITNEY
	li "Líder"           ; MORTY
	li "Líder"           ; CHUCK
	li "Líder"           ; JASMINE
	li "Líder"           ; PRYCE
	li "Líder"           ; CLAIR
	li "Alto Mando"       ; WILL
	li "Alto Mando"       ; KOGA
	li "Alto Mando"       ; BRUNO
	li "Alto Mando"       ; KAREN
	li "Campeón"         ; CHAMPION
	li "Líder"           ; BROCK
	li "Líder"           ; MISTY
	li "Líder"           ; LT_SURGE
	li "Líder"           ; ERIKA
	li "Líder"           ; JANINE
	li "Líder"           ; SABRINA
	li "Líder"           ; BLAINE
	li "Líder"           ; BLUE
	li "Entre. <PK><MN>" ; RED
	li "Entre. <PK><MN>" ; LEAF
	li "Extraño"          ; RIVAL0
	li "Rival"            ; RIVAL1
	li "Rival"            ; RIVAL2
	li "Entre. <PK><MN>" ; LYRA1
	li "Entre. <PK><MN>" ; LYRA2
	li "Joven"        ; YOUNGSTER
	li "Cazabichos"      ; BUG_CATCHER
	li "Campista"           ; CAMPER
	li "Dominguera"        ; PICNICKER
	li "Gemenlos"            ; TWINS
	li "Pescador"        ; FISHER
	li "Ornitólogo"      ; BIRD_KEEPER
	li "Montañero"            ; HIKER
	li "Rocket"           ; GRUNTM
	li "Rocket"           ; GRUNTF
	li "#fan"             ; POKEFANM
	li "#fan"             ; POKEFANF
	li "Oficial"          ; OFFICERM
	li "Oficial"          ; OFFICERF
	li "Enfermera"            ; NURSE
	li "#Maniac"          ; POKEMANIAC
	li "Cosplayer"        ; COSPLAYER
	li "Super Friki"       ; SUPER_NERD
	li "Chica"             ; LASS
	li "Modelo"           ; BEAUTY
	li "Bichomaníaco"       ; BUG_MANIAC
	li "Ruinamaníaco"      ; RUIN_MANIAC
	li "Comefuego"     ; FIREBREATHER
	li "Malabarista"          ; JUGGLER
	li "Estudiante"        ; SCHOOLBOY
	li "Estudiante"       ; SCHOOLGIRL
	li "Médium"          ; PSYCHIC_T
	li "Bruja"       ; HEX_MANIAC
	li "Pensador"             ; SAGE
	li "Medium"           ; MEDIUM
	li "Chica Kimono"      ; KIMONO_GIRL
	li "Anciano"            ; ELDER
	li "Sr.& Jr."         ; SR_AND_JR
	li "Pareja"           ; COUPLE
	li "Caballero"        ; GENTLEMAN
	li "Niño Bien"         ; RICH_BOY
	li "Damisela"             ; LADY
	li "Criapokémon"          ; BREEDER
	li "Pastelera"            ; BAKER
	li "Vaquera"          ; COWGIRL
	li "Marinero"           ; SAILOR
	li "Nadador"          ; SWIMMERM
	li "Nadadora"          ; SWIMMERF
	li "Ladrón"          ; BURGLAR
	li "Agente"            ; PI
	li "Científico"        ; SCIENTIST
	li "Científico"        ; ROCKET_SCIENTIST
	li "Aventurero"          ; BOARDER
	li "Esquiador"            ; SKIER
	li "Cinturón Negro"       ; BLACKBELT_T
	li "Luchadora"      ; BATTLE_GIRL
	li "Domadragón"     ; DRAGON_TAMER
	li "Ingeniero"         ; ENGINEER
	li "Profesora"          ; TEACHER_F
	li "Profesor"          ; TEACHER_M
	li "Guitarrista"        ; GUITARISTM
	li "Guitarrista"        ; GUITARISTF
	li "Motorista"            ; BIKER
	li "Calvo"        ; ROUGHNECK
	li "Domador"            ; TAMER
	li "Artista"           ; ARTIST
	li "Sra. Aromas"       ; AROMA_LADY
	li "Soldado"          ; SOLDIER
	li "Camarero"           ; WAITER
	li "Camarera"         ; WAITRESS
	li "Turista"        ; SIGHTSEERM
	li "Turista"        ; SIGHTSEERF
	li "Veraneante"       ; SIGHTSEERS
	li "Entr. Guay"      ; COOLTRAINERM
	li "Entr. Guay"      ; COOLTRAINERF
	li "Entrs. Guais"          ; ACE_DUO
	li "Veterano"          ; VETERANM
	li "Veterana"          ; VETERANF
	li "Ejecutivo"        ; PROTON
	li "Ejecutivo"        ; PETREL
	li "Ejecutivo"        ; ARCHER
	li "Ejecutivo"        ; ARIANA
	li "Jefe"             ; GIOVANNI
	li "Prof. #mon"       ; PROF_OAK
	li "Prof. #mon"       ; PROF_ELM
	li "Prof. #mon"       ; PROF_IVY
	li "Hombre Mist"      ; MYSTICALMAN
	li "Rey Karateka"      ; KARATE_KING
	li "AmoTorre Jerico"      ; TOWERTYCOON
	li "Jefe Fabrica"       ; FACTORYHEAD
	li "Jessie"           ; JESSIE_JAMES
	li "Ex-Elite"         ; LORELEI
	li "Ex-Elite"         ; AGATHA
	li "Campeón"       ; STEVEN
	li "Investigadora"       ; CYNTHIA
	li "Psíquico"          ; INVER
	li "Entre. <PK><MN>" ; CHERYL
	li "Entre. <PK><MN>" ; RILEY
	li "Entre. <PK><MN>" ; BUCK
	li "Entre. <PK><MN>" ; MARLEY
	li "Entre. <PK><MN>" ; MIRA
	li "Entre. <PK><MN>" ; ANABEL
	li "Galán"            ; DARACH
	li "Señorita"             ; CAITLIN
	li "Líder"           ; CANDELA
	li "Líder"           ; BLANCHE
	li "Líder"           ; SPARK_T
	li "Entrenador"          ; FLANNERY
	li "Luchador"          ; MAYLENE
	li "Nadador"          ; MARLON
	li "Diseñadora"         ; VALERIE
	li "Entre. <PK><MN>" ; KUKUI
	li "Vocalista"         ; PIERS
	li "Empresario"      ; LARRY
	li "Surfista"           ; VICTOR
	li "#Maniac"          ; BILL_T
	li "Entre. <PK><MN>" ; YELLOW
	li "Ex-Líder"        ; WALKER
	li "Estudiante"          ; IMAKUNI
	li "Coleccionista"        ; LAWRENCE
	li "Santuario"           ; REI
	li "Comefuego"     ; FIREBREATHER_ASHES
	assert_list_length NUM_TRAINER_CLASSES
